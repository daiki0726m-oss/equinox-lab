...
def sec_dangerous_favorites(
    ...
    # #94: 複勝率を見れば自明な一般論 (「不安定だから相手を厚く」等) は書かない。
    #   行動指針は下の『共通点(insight)』分析で、母集団比 +15pt 以上の非自明な前走
    #   パターンが見つかった時だけ「該当1人気は割引」と具体的に付ける。
...
def sec_attention_top(
    ...
    include_marks=True: **デッドパス** (#150: リポジトリ全体で呼び出し元ゼロ)。
      印の正本は predictions_cache の mark (点数表・☆妙味枠・△2頭)。
      ここで勝率順に ◎○▲ を振り直すと本番と食い違うので使わないこと。
