...
# v8 (2026-05-16): 市場シグナル完全削除モデルでは AI 勝率が大幅に広がった
# (5/17 分布: win P75=26.4%/max=41.1%, top3 P75=23/max=42, gap P75=7.6, conc P75=54)。
# v7 NORMS (18/40/5/50) では S が 14R/36 と過剰判定 → 上位 10-15% のみ S になる
# よう満点条件を引き上げ再キャリブレーション。
NORMS = {
    'win': 30.0,    # 勝率30%で満点 (v8 P75 26.4% を超える「突出」)
    'top3': 30.0,   # 複勝率30%で満点 (v8 P75 23%)
    'gap': 8.0,     # 8pt差で満点 (v8 P75 7.6pt を超える「圧倒的本命」)
    'conc': 55.0,   # top3合計55%で満点 (v8 P75 54%)
}
