...
- **#40 (2026-06-07) — 投稿スロット排他制御 (atomic lock) を DB に実装、二重投稿を物理的に防ぐ**: ユーザー激怒「4時間前にまた予想を投稿してました。ほんといい加減にしてください。根本的にシステムの予想が反映されない。投稿がちゃんとしよう通りにされない」「cornの問題なら他に方法考えろ」。今日 6/7 (土) の X 投稿 GitHub Actions run を gh CLI で時系列調査すると、`post_predict` が **11:02 (手動) と 17:03 (watchdog 経由) の 2 回**走り、本来 10:15 専用の予想配信が午後にも投稿された。原因: (1) 既存の `post_history.json` がテキストハッシュで重複検知してたが、文面が変わると別ハッシュで擦り抜け、(2) watchdog の cron success 判定が異なる event/title だと「未投稿」と誤判定して fallback 再 trigger、(3) cron/手動/watchdog の 3 系統が独立に走り「この slot は今日まだ投稿されてないか」を atomic に決める仕組みがなかった。**抜本対策**: SQLite に `posted_slots(post_date TEXT, slot_name TEXT, posted_at, run_id, workflow_event, success, UNIQUE(post_date,slot_name))` テーブルを新設し、UNIQUE 制約を「物理的な二重投稿防止」の核に。`database.acquire_post_slot()` が INSERT 試行 → 成功なら lock 取得 → 失敗 (sqlite3.IntegrityError) なら **そのまま skip**。`post_x.py` の 7 cmd_* (post_predict/results/weekday/hit_flash/odds_flash/morning/evening) 冒頭に `_acquire_or_exit('<slot>')` を挿入。`auto_post_x.yml` の Deploy step で **全 post 系 type について keiba.db を git push** するように case 文を拡大 (旧版は post_predict のみ) — これにより次の cron/watchdog/手動 trigger は最新の lock 状態を見て skip 判定できる。`_POST_LOCK_DELEGATED=1` 環境変数で `cmd_hit_flash → cmd_results` の内部委譲時の重複 lock 取得を抑止。`SKIP_POST_LOCK=1` で緊急復旧用 bypass。再投稿が必要な場合は `DELETE FROM posted_slots WHERE post_date=date('now','+9 hours') AND slot_name='<slot>'` で手動解除。**残課題**: 同時刻並走 trigger (cron と watchdog が同一秒に走るケース) は git push race のみで防御 = 完全な atomic ではない。precheck step (投稿直前に origin/main 最新 DB を別ファイルとして fetch して posted_slots 確認) は Phase 2 で追加予定。馬体重スクレイプ未取得問題 (6/7 全 357 頭 weight=0) も別途調査。**教訓: 「気をつける」「重複防止 hash で投稿減らす」では再発する。投稿の冪等性は DB レベルの UNIQUE 制約で物理保証すべき。文面 hash の重複検知は文面が変わると擦り抜けるので invariant にならない。同じ slot を意味する key (date+slot_name) を不変式の中心に置く**。
